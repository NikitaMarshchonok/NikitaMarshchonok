# Nikita Marshchonok

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](http://www.linkedin.com/in/nikita-marshchonok)
[![GitHub](https://img.shields.io/badge/GitHub-000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/NikitaMarshchonok)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:n.marshchonok@gmail.com)


**Data Scientist · ML Engineer · AI** · Israel

I build AI/data products: dashboards, ML models, RAG/LLM features and practical web services. MSc in Software Engineering (SCE).



---

###  Currently working on
- **Data Prism** – automatic BI dashboard + PDF report from CSV/Excel
- **AI DS Agent** – agent that does EDA → feature engineering → model training
- **Weather Outfit Recommender** – Flask app that suggests clothes by weather

---

###  Tech Stack
**Python, SQL, Pandas, scikit-learn, PyTorch, Hugging Face, LangChain, Flask, FastAPI, Docker, Git**

---

###  Selected Projects
- **SCE-Sami-Shamoon-Diploma-Project** – weather→clothes recommender
- **Sales-Forecasting** – RF/XGBoost for time-related sales
- **IMD_Analysis** – NLP movie genre classification
- **Universal_Data_Reporter** – automatic PDF report from CSV

---

### 📊 GitHub Stats
![Nikita's GitHub stats](https://github-readme-stats.vercel.app/api?username=NikitaMarshchonok&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=NikitaMarshchonok&layout=compact&theme=tokyonight)

---

🟢 *Open to DS/ML roles (Israel/remote).*
